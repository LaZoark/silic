...
def setModel(source):
  ...
  messagebox.showinfo(
    "Model Seletion",
    "Model %s was selected including %s sound classes."
    % (model_weight.get(), len(options)),
  )
  for item in options:
...
def readclassfile():
  classes = {}
  df = pd.read_csv(
    "model/%s/soundclass.csv" % model_weight.get(),
    lineterminator="\n",
    encoding="utf-8",
  )
...
def run():
  ...
  text.insert(
    tk.END,
    "SILIC Detector: %s files are found, detecting..... Please wait a moment.\n"
    % len(media_files),
  )
  text.see(tk.END)
  root.update()
  ...
  if all_labels.shape[0] == 0:
    ...
    text.insert(
      tk.END, "Finished. All results were saved in the folder %s\n" % result_path
    )
...
select_input_folder_button = tk.Button(
  root, text="Select Input Folder", command=select_input_folder
)
...
run_button = tk.Button(
  root, text="RUN", bg="#8BC440", fg="#000000", relief="raised", command=run
)
