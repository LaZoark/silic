...
def AudioStandarize(audio_file, sr=32000, device=None, high_pass=0, ultrasonic=False):
  ...
  print(
    ...
    % (
      original_metadata["channel"],
      original_metadata["sample_rate"],
      original_metadata["sample_size"],
      original_metadata["duration"],
    )
  )
  if ultrasonic:
  ...
  print(
    ...
    % (sound.channels, sound.frame_rate, songdata.shape[0], sound.duration_seconds)
  )
...
class Silic:
  ...
  def __init__(
    ...
    self.spec_layer = features.STFT(sr=sr, n_fft=n_fft, hop_length=hop_length).to(
      self.device
    )
    self.spec_mel_layer = features.MelSpectrogram(
      sr=sr,
      n_fft=n_fft,
      n_mels=n_mels,
  ...
  def audio(self, audio_file, ultrasonic=False):
    ...
    (
      ...
      audio_file, self.sr, self.device, high_pass=self.fmin, ultrasonic=ultrasonic
    )

  def save_standarized(self, targetmp3path=None):
    if not targetmp3path:
      targetmp3path = os.path.join(
        self.audiopath, "mp3", "%s.mp3" % self.audiofilename_without_ext
      )
  ...
  def spectrogram(self, audiodata, spect_type="linear", rainbow_bands=5):
    if spect_type in ["mel", "rainbow"]:
      ...
      if spect_type == "mel":
        ...
        for i in range(rainbow_bands):
          subdata = (
            data[
              i
              * int(self.n_mels / rainbow_bands) : (i + 1)
              * int(self.n_mels / rainbow_bands)
            ]
            .cpu()
            .numpy()
          )
          ax[rainbow_bands - i - 1].set_axis_off()
          ax[rainbow_bands - i - 1].pcolormesh(
            subdata,
            cmap=ListedColormap(
              cm.rainbow(
                np.linspace(
                  (i + 1) / rainbow_bands, (i / rainbow_bands), 32
                )
              )
            ),
            rasterized=True,
      ...
      data = (
        torch.sqrt(torch.sqrt(torch.abs(spec[0]) + 1e-6)).cpu().numpy()[:, :, 0]
      )
  ...
  def tfr(
    ...
    if self.clip_length and (
      (self.audiodata.size()[0] / self.sr * 1000) < self.clip_length
    ):
      self.audiodata = torch.cat(
        (
          self.audiodata,
          torch.zeros(
            round(self.clip_length * self.sr / 1000)
    ...
    if not targetfilepath:
      targetfilepath = os.path.join(
        self.audiopath, spect_type, "%s.png" % self.audiofilename_without_ext
      )
      if not os.path.isdir(os.path.dirname(targetfilepath)):
        os.makedirs(os.path.dirname(targetfilepath))
    if not os.path.isdir(os.path.dirname(targetfilepath)):
      print(
        "Error! Cannot find the target folder %s."
    ...
    if (stop - start) / 1000 * self.sr > (max_sample_size):
      ...
      imgs = []
      for ts in range(
        int(round(start / 1000 * self.sr)),
        int(round(stop / 1000 * self.sr) - self.sr * 0.1),
        max_sample_size,
      ):
        ...
        try:
          imgs.append(
            self.spectrogram(data, spect_type, rainbow_bands=rainbow_bands)
          )
        except:
      ...
      self.cv2_img = self.spectrogram(
        self.audiodata[
          int(round(start / 1000 * self.sr)) : int(
            round(stop / 1000 * self.sr)
          )
        ],
        spect_type,
  ...
  def mel_to_freq(self, mel):
    if mel < 0:
      return self.fmin
    mel = mel * (
      1127 * np.log(1 + self.fmax / 700) - 1127 * np.log(1 + self.fmin / 700)
    ) + 1127 * np.log(1 + self.fmin / 700)
  ...
  def detect(
    ...
    for ts in range(0, self.duration, step):
      ...
      if clip_end > self.rainbow_img.shape[1]:
        _silence = np.full(
          (
            self.rainbow_img.shape[0],
            clip_end - self.rainbow_img.shape[1],
            3,
          ),
          255,
        ).astype("float32")
...
def clean_multi_boxes(labels, threshold_iou=0.1, threshold_iratio=0.25):
  ...
  for classid in soundclasses:
    ...
    for i in range(0, df_class.shape[0]):
      check = True
      bb1 = {
        "x1": df_class.loc[i, "time_begin"],
        "x2": df_class.loc[i, "time_end"],
        "y1": df_class.loc[i, "freq_low"],
        "y2": df_class.loc[i, "freq_high"],
      }
      score1 = df_class.loc[i, "score"]
      j = 0
      for j in range(i + 1, df_class.shape[0]):
        bb2 = {
          "x1": df_class.loc[j, "time_begin"],
          "x2": df_class.loc[j, "time_end"],
          "y1": df_class.loc[j, "freq_low"],
          "y2": df_class.loc[j, "freq_high"],
        }
...
def draw_labels(silic, labels, outputpath=None):
  if outputpath and os.path.isdir(outputpath):
    targetpath = os.path.join(
      outputpath, "%s.png" % silic.audiofilename_without_ext
    )
  else:
    if not os.path.isdir(os.path.join(silic.audiopath, "labels")):
      os.makedirs(os.path.join(silic.audiopath, "labels"))
    targetpath = os.path.join(
      silic.audiopath, "labels", "%s.png" % silic.audiofilename_without_ext
    )
...
def browser(
  ...
  for audiofile in audiofiles:
    ...
    if not audiofile.split(".")[-1].lower() in [
      "mp3",
      "wma",
      "m4a",
      "ogg",
    ...
    model.tfr(
      targetfilepath=os.path.join(
        linear_path, model.audiofilename_without_ext + ".png"
      )
    )
    labels = model.detect(
      weights=weights,
      ...
      targetfilepath=os.path.join(
        rainbow_path, model.audiofilename_without_ext + ".png"
      ),
    )
    if len(labels) == 1:
      ...
      newlabels.to_csv(
        os.path.join(lable_path, model.audiofilename_without_ext + ".csv"),
        index=False,
        encoding="utf-8-sig",
      )
      if all_labels.shape[0] > 0:
        all_labels = all_labels = pd.concat(
          [all_labels, newlabels], axis=0, ignore_index=True
        )
      else:
        all_labels = newlabels
      print(
        "%s sounds of %s species is/are found in %s"
  ...
  if all_labels.shape[0] == 0:
    ...
    print(
      "%s sounds of %s species is/are found in %s recording(s). Preparing the browser package ..."
    ...
    with open(
      os.path.join(js_path, "soundclass.js"), "w", newline="", encoding="utf-8"
    ) as csv_file:
    ...
    with open(
      os.path.join(js_path, "labels.js"), "w", newline="", encoding="utf-8"
    ) as f:
      f.write("var  labels  =  [" + "\n")
    ...
    if zip:
      ...
      print(
        "Finished. The browser package is compressed and named result_silic.zip"
      )
    else:
